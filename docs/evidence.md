...
- **Refund-button verification**: read-back of `pay_yRILVDwzGPjc5k` confirms $25 refunded but no `application_fee` object. Both dashboards now distinguish the buyer refund from native fee-return evidence, label derived fees as estimated, and show missing evidence as unverified. The admin dialog's incorrect automatic-reversal claim is removed. The compensation transfer above remains separate evidence.
