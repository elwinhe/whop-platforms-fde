...
Set `WHOP_PLATFORM_COMPANY_ID=biz_BC8sRG36RkIpHk` and your sandbox `WHOP_API_KEY` in the ignored `.env`. The platform and key are created manually. For this script, select `company:create_child` and `company:basic:read` in the sandbox key UI; balance-read is not used here.
...
Offline validation: `node --import tsx --test scripts/seed-sellers.test.ts`.
